...
@section('title', 'Quote ' . $record->reference_number)
    ...
    <table class="meta-table">
        <tr>
            ...
            <td width="50%" class="text-right">
                <h2 style="margin: 0; color: #444;">QUOTE</h2>
                <strong>Reference:</strong> {{ $record->reference_number }}<br>
                <strong>Date:</strong> {{ $record->created_at->format('d M, Y') }}<br>
                <strong>Valid Until:</strong> {{ $record->valid_until ? $record->valid_until->format('d M, Y') : 'N/A' }}<br>
